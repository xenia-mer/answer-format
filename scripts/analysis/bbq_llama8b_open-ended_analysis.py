...
df_dis = pd.read_csv("data/bbq_analysis/bbq_open/bbq_llama8b_open-ended_responses_labelled_disambig.csv", sep=",")
df_b = df_dis.copy()

df_b[['response_label', 'bias']] = df_b.apply(lambda row: pd.Series(label_and_bias(row)), axis=1)
df_b.to_csv("data/bbq_analysis/bbq_open/bbq_llama8b_open-ended_responses_labelled_bias_disambig.csv", index=False)

df_b_labels = df_b[(df_b['example_id'] >= 0) & (df_b['example_id'] <= 671)]
df_b_names = df_b[(df_b['example_id'] >= 833) & (df_b['example_id'] <= 5559)]

df_am = pd.read_csv("data/bbq_analysis/bbq_open/bbq_llama8b_open-ended_responses_labelled_ambig.csv", sep=",")
df_a = df_am.copy()

df_a[['response_label', 'bias']] = df_a.apply(lambda row: pd.Series(label_and_bias(row)), axis=1)
df_a.to_csv("data/bbq_analysis/bbq_open/bbq_llama8b_open-ended_responses_labelled_bias_ambig.csv", index=False)
