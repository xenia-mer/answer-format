...
df_dis = pd.read_csv("data/bbq_analysis/bbq_closed-ended/bbq_mistral7b_closed-ended_responses_labelled_disambig.csv", sep=",")
df_b = df_dis.copy()

df_b[['response_label', 'bias']] = df_b.apply(lambda row: pd.Series(label_and_bias(row)), axis=1)
df_b.to_csv("data/bbq_analysis/bbq_closed-ended/bbq_mistral7b_closed-ended_responses_labelled_bias_disambig.csv", index=False)

df_b_labels = df_b[(df_b['example_id'] >= 0) & (df_b['example_id'] <= 671)]
df_b_names = df_b[(df_b['example_id'] >= 833) & (df_b['example_id'] <= 5559)]

df_am = pd.read_csv("data/bbq_analysis/bbq_closed-ended/bbq_mistral7b_closed-ended_responses_labelled_ambig.csv", sep=",")
df_a = df_am.copy()

df_a[['response_label', 'bias']] = df_a.apply(lambda row: pd.Series(label_and_bias(row)), axis=1)
df_a.to_csv("data/bbq_analysis/bbq_closed-ended/bbq_mistral7b_closed-ended_responses_labelled_bias_ambig.csv", index=False)
...
print("===By answer_order===")

answer_orders = sorted(df_b['answer_order'].dropna().unique())
    
for order in answer_orders:
    print(f"Answer order: {order}")
        
    for subset_name, (df_b_sub, df_a_sub) in subsets.items():
        df_b_order = df_b_sub[df_b_sub['answer_order'] == order]
        df_a_order = df_a_sub[df_a_sub['answer_order'] == order]
            
        sDIS_order = compute_sDIS(df_b_order)
        acc_order = compute_accuracy(df_a_order)
        sAMB_order = (1 - acc_order) * sDIS_order if (acc_order is not None and sDIS_order is not None) else None
            
        print(f"    sDIS {subset_name}: {sDIS_order:.6f}")
        print(f"    Accuracy {subset_name}: {acc_order:.6f}")
        print(f"    sAMB {subset_name}: {sAMB_order:.6f}")
            
        boot_sDIS_order = bootstrap_sDIS(df_b_order)
        print("    ", format_ci(f"sDIS {subset_name}", boot_sDIS_order))
            
        boot_sAMB_order = bootstrap_sAMB(df_b_order, df_a_order)
        print("    ", format_ci(f"sAMB {subset_name}", boot_sAMB_order))