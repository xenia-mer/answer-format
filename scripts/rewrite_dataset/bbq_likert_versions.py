...
def bbq_likert_versions(input_file):
    ...
    for points, polarities in likert_configs.items():
        for polarity, (question_suffix, scale) in polarities.items():
            ...
            new_df['likert_scale_question'] = new_df['likert_scale_question'].apply(update_question)
            new_df['scale'] = scale
            output_file = (f"data/bbq/bbq_likert/bbq_gender_likert_{points}_{polarity}.csv")
            new_df.to_csv(output_file, index=False, sep=';')
            print(f"Created: {output_file}")
    print("\nAll files created.")

if __name__ == "__main__":
    input_file = "data/bbq/bbq_gender_likert_4_neg.csv"
    bbq_likert_versions(input_file)