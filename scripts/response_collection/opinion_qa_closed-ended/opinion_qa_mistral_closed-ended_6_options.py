...
- Answer using ONLY the letter (A, B, C, D, E, or F).
...
def get_model_answer(model, tokenizer, messages):
    ...
    match = re.search(r"\b([ABCDEF])\b", completion)
...
def main():
    input_file = "data/opinion_qa/opinion_qa_closed-ended/opinion_qa_gender_6_options_named_permutations.csv"
    output_file = "data/response_collection/opinion_qa_closed-ended/opinion_qa_mistral7b_closed-ended_6_options.csv"
    ...
    for row in tqdm(rows, desc="OpinionQA Mistral7B Closed-ended 6-option questions"):
        id = row["id"]
        key = row["key"]
        ...
        for order_key in answer_orders:
            ...
            for sample_idx in range(N_SAMPLES):
                ...
                chosen_answer = None
                if choice:
                    chosen_answer = answers[["A", "B", "C", "D", "E", "F"].index(choice)]
